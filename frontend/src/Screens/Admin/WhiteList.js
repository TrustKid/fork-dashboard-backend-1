...
import {SET_THEME,PROJECT_CATEGORY, SET_WHITELIST_DATA} from "../../actionTypes";
 ...
 useEffect(() => {
    ...
    const onAdd = (router) => {
        ...
        if(projectInput.current.value=='')setProjectErr(true)
            ...
            // getProjectData[aa.map(item=>{return item.fork}).indexOf(projectInput.current.value)].image
            let data = {
            ...
            .then(res=>{
                if(res.data)
                // let num = getWhitelistdata.map(item=>{return item._id}).indexOf(res.data._id)
                switch (router){
