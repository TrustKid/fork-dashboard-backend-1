...
const Portfolio = (data) => {
    ...
    //   }
      ...
      const addClass_collapsed = () => {
          ...
          for(var i=0;i<element.length;i++)
          element_1[i]&&element_1[i].classList.add("collapsed");
      }
      ...
      const onFilterType=(value)=>{
          ...
          portfolioData.forEach((item, index) => {
              if(protocolValue!='all'){
                if(value=='all'){
                    if (item.chain.toLowerCase().includes(protocolValue)){
                        preTable_data.push(item);
                } }
                else{
                    if (item.type.toLowerCase().includes(value)
                  && item.chain.toLowerCase().includes(protocolValue)){
                    preTable_data.push(item);
                          ...
                          {portfolio_table_body_data.map((item, index) => {
                              return <><tr key={index} className="table-tr accordion-toggle " 
                                  ...
                                  <td className="table-body table-body-protocol" title={item.protocol_url}>
                                      ...
                                      <img className="table-td-m-image table-td-m-image-margin" src={item.type.toLowerCase()=='wallet'?item.chain_logo:item.protocol_logo?item.protocol_logo:'https://assets.debank.com/static/media/default.b2295297.svg'} />{item.protocol}
                                      </a></td>
                                  <td className="table-body d-flex">
                                      <div  className="table-assets-item d-flex">
                                          {item.assets.map((item4,index4)=>{return <img key={index4}className="table-td-m-image" src={item4.logo?item4.logo:'https://assets.debank.com/static/media/default.b2295297.svg'} />})}
                                  ...
                                  <td className="table-body-num" style={{ color: (item.value-item.cost_basis)/item.cost_basis < 0 ? '#dd3279' : 'white' }}>{item.cost_basis==0?'-':((item.value-item.cost_basis)/item.cost_basis*100).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")+'%'}</td>
                                </tr>
